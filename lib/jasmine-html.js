...
function toJson(obj,accept) {

    var exceptStr ='';
    if(accept && accept.length){
        exceptStr  = accept.join('|');
    }
    switch (typeof obj) {
        case 'object':
            if (obj) {
                ...
                if (obj instanceof Array) {
                    for (var i=0;i < obj.length;i++) {
                        list.push(toJson(obj[i],accept));
                    }
                    return '[' + list.join(',') + ']';
                } else {
                    for (var prop in obj) {
                        if(obj.hasOwnProperty(prop)&& exceptStr.lastIndexOf(prop) !== -1){
                            list.push('"' + prop + '":' + toJson(obj[prop],accept));
                        }
...
jasmine.HtmlReporter = function(_doc) {
  ...
  self.reportRunnerResults = function(runner) {
    var resultJson = toJSON(runner);
/*     console.log(JSON.stringify(resultJson));*/
      window.postmsg.send({target:window.parent,data:{"head":'resultJSON',"body":JSON.stringify(resultJson)}});
...
jasmine.HtmlReporter.ReporterView = function(dom) {
 ...
 /*  var resultHTML=KISSY.DOM.get('#HTMLReporter').innerHTML;
   window.postmsg.send({target:window.parent,data:{"head":'result',"body":resultHTML}});*/
  };
